...
# help functions to process sets of sprites
def process_sprite_group(sprite_group, canvas):
    sprite_group_copy = set(sprite_group)
    for sprite in sprite_group:
        if sprite.update():
            sprite_group_copy.remove(sprite)
        sprite.draw(canvas)
    sprite_group.intersection_update(sprite_group_copy)
...
def group_group_collide(group1, group2):
    ...
    for sprite in group1_copy:
        if group_collide(group2, sprite):
            group1_copy.remove(sprite)
            collisions += 1
    group1.intersection_update(group1_copy)
    return collisions
...
def rock_spawner():
    ...
    if len(rock_group) < 12 and started:
        ...
        rock_vel = [(random.random() * .6 - .3 ) * score / 3.0, (random.random() * .6 - .3 ) * score / 3.0]
...
# init game
def init():
    global score, lives, time, started, my_ship, rock_group, missile_group
